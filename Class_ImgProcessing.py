...
class ImageProcessing:
    ...
    def display_img(image_array: ndarray, text: str = '', position: Literal['center', 'left', 'right'] = 'center') -> None:
        ...
        if text != '':
            plt.title(text, loc=position)

        plt.axis('off')     # No axis for images
        plt.tight_layout(pad=0)     # No padding 
        plt.show(block=False)       # Image window is shown immediately, not after the program ends
        return
    ...
    def convolve2d(image_array: ndarray, kernel: ndarray) -> ndarray:
        ...
        padded_img = np.pad(image_array, ((padrows, padrows), (padcols, padcols)), 'constant', constant_values=0.0)
    ...
    @staticmethod
    def convolve3d_scipy(image_array: ndarray, kernel: ndarray) -> ndarray:
        """
        Applies a 3D convolution to an image array using a specified kernel with SciPy.
        ...
        if image_array.ndim == 2:
            ...
            for c in range(channels):
                output[:,:,c] = convolve(image_array[:,:,c], kernel, mode='reflect')
    ...
    def edge_detection(self, kernel_x: ndarray = sobel_x, kernel_y: ndarray = sobel_y) -> ndarray:
        """
        ...
        if self.arr.ndim == 3:
            ...
            self.arr = np.dot(self.arr, ImageProcessing.gray_convert_arr)

        self.arr = self.arr.astype(np.float32)
        edge_x = np.abs(ImageProcessing.convolve3d_scipy(self.arr, kernel_x))
        edge_y = np.abs(ImageProcessing.convolve3d_scipy(self.arr, kernel_y))
        ...
        self.arr = np.hypot(edge_x, edge_y).astype(np.uint8)    #Combining the edges in the x and y directions
        return self.arr
